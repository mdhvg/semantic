            ...
            try {
                if (searchQuery.length) {
                    const response = await fetch(
                        baseUrl + endpoints.searchDocument + "/" + searchQuery,
                        {
                            method: "GET",
                            headers: {
                                cors: "no-cors",
                    ...
                    if (response.ok) {
                        ...
                        throw new Error(
                            `Server is down with status ${response.status}`,
                        );
                    }
                }
            } catch (error) {
...
>
    <div class="w-full h-full flex flex-row items-center px-3 gap-1">
        ...
        />
        <SearchIcon
            class="group-hover/search:fill-current h-2/3 fill-muted-foreground p-1"
        />
    </div>
        ...
        >
            {#each searchResultList as result, index}
                <Button
                    ...
                    {index === searchResultList.length - 1
                        ? 'rounded-b-lg'
                        : ''}
                        "
                >
