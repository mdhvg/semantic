...
function LinuxSetup() {
	...
	if (!fs.existsSync(miniforgeSetup)) {
		try {
			...
			// download miniforge
			execSync(`curl "https://github.com/conda-forge/miniforge/releases/latest/download/${miniForgeScriptName}" -o ${miniforgeSetup}`, { encoding: 'utf-8' });
	...
	if (!fs.existsSync(join(envPrefix, "envs", environmentName))) {
		execSync(`bash ${linuxSetupPath}/create_env.sh ${envPrefix} ${environmentPath}`, { encoding: 'utf-8' });
	}
	console.log("Environment created");

	// Run the server
	console.log("Starting server...");
	execSync(`bash ${linuxSetupPath}/run_server.sh ${envPrefix} ${environmentName} ${backendPath}`, { encoding: 'utf-8', stdio: 'inherit' });
}
